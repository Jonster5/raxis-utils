...
export type QueryItem = CompType | typeof Entity | QueryProcesser;

type ResultFrom<I extends QueryItem> = I extends typeof Entity
	? Entity
	: I extends QueryProcesser<infer R>
		? R
		: I extends CompType<infer C>
			? C
			: never;

type ExtractResultsFrom<Q extends Query> =
	Q extends Query<infer R extends [QueryItem]>
		? ResultFrom<R[0]>
		: Q extends Query<infer R extends [QueryItem, ...QueryItem[]]>
			? { [K in keyof R]: ResultFrom<R[K]> }
			: never;

export class Query<Items extends [QueryItem, ...QueryItem[]] = [QueryItem, ...QueryItem[]]> {
	...
	matches(other: Query): boolean {
		...
		for (let i = 0; i < this.items.length; i++) {
			...
			if (
				(a instanceof QueryProcesser && !(b instanceof QueryProcesser)) ||
				(b instanceof QueryProcesser && !(a instanceof QueryProcesser))
			) {
				return false;
			}

			if (a instanceof QueryProcesser && b instanceof QueryProcesser) {
	...
	static DefineParameter<Args extends unknown[]>(validator: QueryValidator<Args>): QueryParameter<Args> {
...
export class QueryProcesser<R extends NonNull = NonNull> {
	constructor(
	...
	matches(other: QueryProcesser): boolean {
...
export class QueryFilter {
	constructor(
		private fn: QueryValidator<unknown[]>,
		private args: unknown[]
	) {}
...
export class QueryResults<Q extends Query = Query> {
	...
	private getResult<I extends QueryItem>(item: I, eid: Eid): ExtractResultsFrom<Q> {
		...
		} else if (item instanceof QueryProcesser) {
			return item.process(eid, this.creg, this.ereg) as ExtractResultsFrom<Q>;
		} else {
			return this.creg.get(eid, item).expect('An existing component') as ExtractResultsFrom<Q>;
		}
	}
	...
	validate(eid: Eid) {
		for (const item of this.query.items) {
			if (item === Entity) continue;
				...
				if (!item.validate(eid, this.creg, this.ereg)) {
...
export const Optionally = Query.DefineModifier<[CompType], Option<Component>>(
...
) as <T extends Component>(type: CompType<T>) => QueryProcesser<Option<T>>;
